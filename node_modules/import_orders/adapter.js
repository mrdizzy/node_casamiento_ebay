    ...
    db.get('time', function(err, res) { // Get last imported time and begin import
        if (err) {
            ...
            eBayAPI.makeRequest("GetOrders", function(err,res) {
                    GetOrdersFromEbay(err,res, cb, db);
                }, {
...
function GetOrdersFromEbay(err, res, cb, db) {
    ...
    parsedOrders.forEach(function(order) {
        db.save(order, function(error, response) {
            if (err) {
                ...
                waiting--;
                if (!waiting) {
                    updateTime(res.Timestamp, lineItemIDs, cb, db);
...
function updateTime(timeStamp, lineItemIDs, cb, db) {
    db.save('time', {
        ...
        if (err) {
            ...
            combineOrders(lineItemIDs, cb, db);
        }
    });
}
function combineOrders(lineItemIDs, cb, db) {
