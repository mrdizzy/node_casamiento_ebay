...
addBatch({
    "When importing an order": {
        ...
        "after polling the database": {
            ...
            "the lastModified time in the database should be updated": function(topic) {
    ...
    "When importing two orders each with a single transaction": {
        topic: function() {
            var self = this;
            fakeResponse("two_orders_b", function() {
                AddOrdersToDatabase(db, eBayAPI, self.callback);
            })
        },
        "after polling the database": {
            ...
            "both orders should be added successfully": function(topic) {
    ...
    "When importing two orders each with a single transaction": {
        ...
        "then importing a single order with a two transactions, one of which has the same OrderLineItemID as one of the existing orders": {
            topic: function() {
                var self = this;
                fakeResponse("order_c_combine_with_order_b", function() {
                    AddOrdersToDatabase(db, eBayAPI, self.callback);
                })
            },
            "after polling the database": {
                ...
                "the orders should have been combined": function(topic) {
    ...
    "When importing two orders each with a single transaction": {
        ...
        "then importing two orders each with two transactions, one transaction of each which has the same OrderLineItem ID of each of the existing orders": {
            topic: function() {
                var self = this;
                fakeResponse("two_orders_d_combine_with_orders_b", function() {
                    AddOrdersToDatabase(db, eBayAPI, self.callback);
                })
            },
            "after polling the database": {
